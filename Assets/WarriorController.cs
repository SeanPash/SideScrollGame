...
IEnumerator DoParry()
{
    ...
    // Wait for the remainder of parry window
    yield return new WaitForSeconds(0.18f); // 0.2f total parry window

    parryHitbox.gameObject.SetActive(false);

    yield return new WaitForSeconds(0.2f); // lockout after window
